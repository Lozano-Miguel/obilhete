...
- **External APIs:** Gemini Flash (AI logic), TMDB API (Film metadata enrichment)
...
5. **AI Processing:** Submits the enriched user profile to Gemini to generate the personality tag and recommendations (filtering out previously watched films).
...
- **AI Constraints:** All Gemini outputs (personality tags and recommendation reasoning) are forced into Portuguese via prompt engineering. The system enforces strict JSON outputs. The active model fallback chain is `gemini-2.5-flash` -> `gemini-2.5-flash-lite` -> `gemini-1.5-flash`.
...
- `POST /api/recommendations` - Refreshes only the Gemini recommendations utilizing previously cached film data.
